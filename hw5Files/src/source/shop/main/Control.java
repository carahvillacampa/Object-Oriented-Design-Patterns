...
import source.shop.ui.UI;
import source.shop.ui.UIMenu;
import source.shop.ui.UIMenuAction;
import source.shop.ui.UIMenuBuilder;
import source.shop.ui.UIError;
import source.shop.ui.UIForm;
import source.shop.ui.UIFormTest;
import source.shop.ui.UIFormBuilder;
import source.shop.data.Data;
import source.shop.data.Inventory;
import source.shop.data.Video;
      ...
      new UIMenuAction() {
        public void run() {
        	if(_inventory.size() > 0) {
        		Iterator<Record> recordIter= _inventory.iterator(new Comparator<Record>() {
					@Override
					public int compare(Record o1, Record o2) {
						return o1.numOut()-o2.numOut();
					}	
        		});
        		StringBuilder build= new StringBuilder();
        		int count=1;
        		while(recordIter.hasNext() && count < 11) {
        			Record r= recordIter.next();
        			build.append(" "+ r.video().title() + " [" + r.numOwned() + "]\n");
        			count++;
        		}
