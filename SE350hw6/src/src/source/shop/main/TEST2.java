...
import src.source.shop.command.RerunnableCommand;
import src.source.shop.command.UndoableCommand;
...
public class TEST2 extends TestCase {
  ...
  public void test1() {
    ...
    assertTrue  (!Data.newOutCmd(inventory, v1).run()); //fail
    assertEquals( "K (2003) : S [2,2,2]", inventory.get(v1).toString() );
    assertTrue  ( UNDO.run() );
    assertEquals( "K (2003) : S [2,1,1]", inventory.get(v1).toString() ); //fail
    assertTrue  ( UNDO.run() );
    assertEquals( "K (2003) : S [2,0,0]", inventory.get(v1).toString() );
    assertTrue  ( REDO.run() );
    assertEquals( "K (2003) : S [2,1,1]", inventory.get(v1).toString() );
